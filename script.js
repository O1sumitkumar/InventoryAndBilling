...
function finalBill() {
  const finalBillPrice = priceArray.reduce(
  ...
  document.getElementById("finalPrice").innerHTML = `₹ ${Math.floor(
    finalPrice
  )}`;
...
function save() {
  ...
  if (tdList.length <= 0) {
    ...
    doc
      ...
      .setFontSize(18)
      .setTextColor("#3C3C3C")
      .text("The Indian Mart", 14, 22);
    doc
      .setFontSize(12)
      .text(`Customer Name: ${invoiceData.customer.name}`, 14, 40)
    ...
    doc
      ...
      .text("ID", 14, 80)
      .text("Category", 24, 80)
      .text("Product Name", 60, 80)
      .text("Quantity", 107, 80)
      .text("Product MRP", 136, 80)
    ...
    for (const item of invoiceData.item) {
      doc
        .setFontSize(10)
        .setTextColor("#3C3C3C")
        .text(item.id.toString(), 14, y)
        .text(item.Category.toString(), 24, y)
        .text(item.productName.toString(), 60, y)
        .text(item.unit.toString(), 107, y)
        .text(item.mrp.toString(), 136, y)
        .text(item.price.toString(), 176, y);
      y += 8;
    }
    doc
      .setFontSize(16)
      .text(` Total:  $${invoiceData.total.toFixed(2)}`, 14, y + 16);
